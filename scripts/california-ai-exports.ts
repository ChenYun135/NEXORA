...
await promote("relationships.csv", csv(["from_institution_id","to_institution_id","work_count","period","status","source"], openAlex.relationships.map((row) => [row.fromInstitutionId,row.toInstitutionId,row.workCount,"2015-2026","VERIFIED_RELATIONSHIP",row.sourceUrl])));
...
console.log(JSON.stringify({ status: "PASS", directory: target, files: 6, caseVersion: openAlex.caseVersion, snapshotDate: openAlex.snapshotDate }, null, 2));
