...
test("ships 27 bilingual demo records across eight jurisdictions and nine industries",()=>{
 assert.equal(policyRecords.length,27);assert.equal(new Set(policyRecords.map(p=>p.jurisdictionId)).size,8);assert.equal(policyIndustries.length,9);assert.ok(policyRecords.every(p=>p.localizedTitle.en&&p.localizedTitle.zh&&p.isDemo));
});
...
test("parses cross-module query state",()=>{assert.deepEqual(parsePolicyQuery("?jurisdiction=united-states&industry=semiconductors&technology=advanced-packaging&policy=us-chip-workforce"),{jurisdictionId:"united-states",industryId:"semiconductors",technologyId:"advanced-packaging",policyId:"us-chip-workforce"});});
