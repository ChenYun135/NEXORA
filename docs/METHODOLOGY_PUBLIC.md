# Public Methodology and Limitations

NEXORA answers four different questions with four explicitly separated methods:

1. **What is observed?** Public provider records with source, retrieval/freeze period, geography, and coverage.
2. **What is derived?** Deterministic normalization, counts, rates, networks, or scores with declared inputs.
3. **What can be interpreted?** Evidence-first synthesis limited to retrieved, citeable support; insufficient evidence triggers qualification or refusal.
4. **What might happen under assumptions?** Scenario trajectories from transparent equations and parameters; simulation is not prediction.

Cross-provider values are not automatically comparable. Geography, topic taxonomy, document type, time window, institutional coverage, and source revisions can differ. Network edges indicate recorded relationships within the selected frame, not importance or the complete ecosystem. Policy presence does not prove implementation or effect. Organization attributes do not imply endorsement, quality, creditworthiness, or investment value.

Patent, entrepreneurship, or capital layers remain unavailable or unsupported where public evidence is insufficient.
