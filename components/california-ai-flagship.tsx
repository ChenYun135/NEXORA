...
const copy = {
  en: {
    private: "PUBLIC EVIDENCE SHOWCASE", case: "FLAGSHIP CASE", evidence: "EVIDENCE-BASED CASE", snapshot: "VERSIONED DATA SNAPSHOT", updated: "Versioned",
  ...
  zh: {
    private: "公开证据展示", case: "旗舰案例", evidence: "循证案例", snapshot: "版本化数据快照", updated: "版本化",
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.section}>
      ...
      <div className={styles.researchGrid}>
        <article className={styles.chartPanel}><header><h3>{lang === "en" ? "2015—2025 complete · 2026 partial" : "2015—2025 完整年度 · 2026 部分年度"}</h3><span>{t.sourceNative}</span></header><div className={styles.annualChart} role="img" aria-label={lang === "en" ? "Annual OpenAlex work counts from 2015 through incomplete 2026" : "2015 年至尚未完整的 2026 年 OpenAlex 年度成果数"}>{californiaAIAnnual.map((row) => <div key={row.year} className={row.incomplete ? styles.incomplete : ""}><i style={{ height: `${Math.max(6, row.workCount / maxAnnual * 100)}%` }}/><b>{row.workCount.toLocaleString("en-US")}</b><span>{row.year}</span></div>)}</div><p className={styles.warning}>△ {t.warning2026}</p></article>
    ...
    <section className={styles.section}>
      ...
      <div className={empiricalStyles.empiricalGrid}>
        <article className={styles.panel}><h3>{lang === "en" ? "AI-adjacent workforce · San Jose-Sunnyvale-Santa Clara MSA" : "AI 邻近劳动力 · 圣何塞—森尼韦尔—圣克拉拉都会统计区"}</h3><p className={styles.caution}>{californiaAITalentLimitations[lang]}</p>{californiaAITalentIndicators.map((item) => <div className={empiricalStyles.empiricalRow} key={item.id}><div><b>{item.label[lang]}</b><small>{item.soc} · MAY 2025</small></div><strong>{item.employment.toLocaleString("en-US")}</strong><span>LQ {item.locationQuotient}</span></div>)}</article>
    ...
    <section className={styles.section}>
      <SectionHead eyebrow="RADAR + ECOSYSTEMS / CASE-SPECIFIC" title={lang === "en" ? "06 / Empirical signal profile" : "06 / 实证信号画像"} subtitle={lang === "en" ? "A context-only cross-layer panel plus within-frame network metrics; no combined evidence index." : "仅作背景的跨层面板与框架内网络指标；不再计算综合证据指数。"}/>
      <div className={empiricalStyles.signalGrid}><article className={styles.metricPanel}><span>{californiaAIRadarProfile.status.replaceAll("_", " ")}</span><strong>—</strong><h3>{lang === "en" ? "Cross-layer context panel" : "跨层背景面板"}</h3><p>{lang === "en" ? californiaAIRadarProfile.warning : "这些信号的地理、构念与时间覆盖不兼容，只能并列作为背景；NEXORA 不计算综合分数、排名、相关性或因果效应。"}</p><small>{lang === "en" ? "NOT DIRECTLY COMPARABLE" : "不可直接比较"}</small></article><article className={styles.panel}>{californiaAIRadarProfile.signals.map((signal) => <div className={empiricalStyles.signalRow} key={signal.id}><span>{signal.id.replaceAll("-", " ")}</span><i><em style={{width:`${signal.score}%`}}/></i><b>{signal.score}</b><small>{lang === "en" ? "DIAGNOSTIC" : "诊断值"}</small></div>)}</article><article className={styles.panel}><h3>{lang === "en" ? "Network centrality" : "网络中心性"}</h3>{californiaAINetworkMetrics.slice(0, 6).map((metric) => <div className={empiricalStyles.empiricalRow} key={metric.institutionId}><div><b>{metric.name}</b><small>{metric.scopeLabel}</small></div><strong>{metric.degree}</strong><span>Σ {metric.weightedDegree}</span></div>)}</article></div>
    ...
    <section id="sources" className={styles.section}>
      ...
      <div className={styles.methodGrid}><Link href="/methodology"><span>01</span><h3>{t.methods}</h3><p>{lang === "en" ? "Public methodology, evidence states and analytical limits." : "公开方法、证据状态与分析边界。"}</p></Link><Link href="/data-status"><span>02</span><h3>{t.provenance}</h3><p>{lang === "en" ? "Provider, coverage, freshness and public data status." : "数据来源、覆盖范围、时效性与公开状态。"}</p></Link><Link href="/methodology"><span>03</span><h3>{t.dictionary}</h3><p>{lang === "en" ? "Public definitions for evidence and derived indicators." : "公开证据与衍生指标定义。"}</p></Link><Link href="/methodology"><span>04</span><h3>{t.reproduce}</h3><p>{lang === "en" ? "Public reproducibility principles and limitations." : "公开复现原则与局限。"}</p></Link></div>
    ...
    <footer className={styles.footer}><div><b>NEXORA</b><span>CALIFORNIA AI FLAGSHIP · {californiaAIRegistry.version}</span></div><p>{lang === "en" ? "Selected evidence panel, not a statewide census · Scenario result, not forecast · Evidence status shown at point of use" : "精选证据面板，并非全州普查 · 情景结果而非预测 · 证据状态在使用处标明"}</p><button onClick={() => window.print()}>{t.print}</button></footer>
  </main>;
}
