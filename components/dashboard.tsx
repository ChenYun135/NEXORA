...
export function Dashboard(){
 ...
 return <main className={`${styles.page} ${lang==="zh"?styles.zh:""}`}>
  ...
  <section className={styles.principles}><div className={styles.sectionHead}><span>02 / RESEARCH PRINCIPLES</span><h2>{lang==="en"?"Intelligence with visible boundaries":"边界清晰的研究情报"}</h2><p className={styles.trustLine}>{lang==="en"?"One evidence trail, eight connected views · NO LIVE GEOLOCATION · DEMO COMPOSITE MODEL":"一条证据链，八个相互连接的研究视图 · 不使用实时地理定位 · 综合模型为演示状态"}</p></div><div>{principles.map((item,i)=><article key={item.en[0]}><span>0{i+1}</span><h3>{item[lang][0]}</h3><p>{item[lang][1]}</p></article>)}</div></section>
 </main>;
}
