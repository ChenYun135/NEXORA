...
export function DataStatus(){
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.intro}><span>PUBLIC DATA FOUNDATION / R1</span><h1>{t.title}</h1><p>{t.lead}</p><div className={styles.summary}><article><b>{summary.total}</b><span>{t.coverage}</span></article><article><b>{summary.tierA}</b><span>{t.tierA}</span></article><article><b>{summary.tierAReady}</b><span>{t.ready}</span></article><article><b>{summary.byStatus.STAGED}</b><span>{t.staged}</span></article></div><p className={styles.boundary}>{t.boundary}</p></section>
