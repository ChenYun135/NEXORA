...
export const providerRequestPlans={
  nsfAwards:(state:string,year:number):ProviderRequestPlan=>({providerId:"NSF_AWARDS",officialOrigin:"https://www.research.gov",path:"/awardapi-service/v1/awards.json",query:{fundProgramName:"",awardeeStateCode:requireCode(state,/^[A-Z]{2}$/,"STATE"),dateStart:`01/01/${year}`,dateEnd:`12/31/${year}`},authentication:"NONE",responseFormat:"JSON"}),
  censusBfs:(year:number,state="*"):ProviderRequestPlan=>({providerId:"CENSUS_BFS",officialOrigin:"https://api.census.gov",path:"/data/timeseries/bfs",query:{get:"BA,HBA,WBA,CBA,BF4Q,BF8Q",for:`state:${requireCode(state,/^(?:\*|\d{2})$/,"STATE_FIPS")}`,time:String(year)},authentication:"OPTIONAL_KEY",responseFormat:"JSON"}),
  ...
  censusAcs:(year:number,variables:string[],state="*"):ProviderRequestPlan=>({providerId:"CENSUS_ACS",officialOrigin:"https://api.census.gov",path:`/data/${year}/acs/acs5`,query:{get:variables.map(value=>requireCode(value,/^[A-Z]\d{5}_\d{3}[EM]$/,"ACS_VARIABLE")).join(","),for:`state:${requireCode(state,/^(?:\*|\d{2})$/,"STATE_FIPS")}`},authentication:"OPTIONAL_KEY",responseFormat:"JSON"}),
